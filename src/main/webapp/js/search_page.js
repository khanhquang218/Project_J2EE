				...
				await this.fetchDataSearchPageAll(valueParam).then(async () => {
					await that.innerListFriend().then((result) => {
						if (that.listSearchFriendReult && that.listSearchFriendReult.length > 0) {
							$("#search_page-friend-list-wrapper").innerHTML = result + `<a href="/Project_J2EE/Search.jsp?page=user&value=${valueParam}"><button class="btn search_page-btn_view_all_with_friend">Xem
				tất cả</button></a>`;
						} else {
